...
class KGEModel(nn.Module):
    ...
    def TestE(self, head, relation, tail, mode):
        ...
        if mode=='head-batch':
            ...
            phase1 = head1 + relation1 - tail1
            phase2 = head2 + relation2 - tail2
        score1 = torch.sum(torch.abs(torch.sin(phase1 / 2)), dim=2)
        score2 = torch.sum(torch.abs(torch.sin(phase2 / 2)), dim=2)
        score = self.gamma.item() - (score1 * 0.5 + score2) * self.modulus
        return score
    #########################################################
    ...
    def CylinderE(self,head, relation, tail, mode):
        ...
        m_score = (h_z * r_z.abs() - t_z).norm(p=2, dim=2)

        if mode == 'head-batch':
            phase = head_phase + (rel_phase - tail_phase)
        else:
            phase = head_phase + rel_phase - tail_phase
        p_score = torch.norm(torch.abs(torch.sin(phase / 2)), p=1, dim=2)
        score = m_score * self.m_weight + p_score * self.modulus * 0.5
