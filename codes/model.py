...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        self.epsilon = 2.0
        self.m_weight = nn.Parameter(torch.Tensor([[5.5]]))
    ...
    def TestE(self, head, relation, tail, mode):
        ...
        score = self.gamma.item() - (intensity+0.00000001).sqrt().sum(dim=2)
        return score
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        # print(negative_score)
        thre = 1.0
        negative_score1 = torch.where(negative_score > thre, negative_score * 0.5, negative_score)
        if args.negative_adversarial_sampling:
            # In self-adversarial sampling, we do not apply back-propagation on the sampling weight
            # detach() 函数起到了阻断backpropogation的作用
