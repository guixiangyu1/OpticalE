...
class KGEModel(nn.Module):
    ...
    def OpticalE_dir_ampone(self, head, relation, tail, mode):
        ...
        if mode == 'single' or mode == 'head-batch-test' or mode == 'tail-batch-test':
            ...
            # 非相关负例
            intensity = -0.1 * torch.abs(torch.cos(head_dir - tail_dir)) + 2.0
        elif mode == 'relation-batch':
            # 相关负例
            intensity = 0.1 * torch.cos(head_phase + relation - tail_phase) + 2.0
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        log = {
            **regularization_log,
            ...
            'negative_sample_loss_relevant': negative_sample_loss_relevant.item(),
            'negative_sample_loss_unrelevant': negative_sample_loss_unrelevant.item(),
            'loss': loss.item()
        }
