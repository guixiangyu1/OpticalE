...
class KGEModel(nn.Module):
    ...
    def pOpticalE(self, head, relation, tail, mode, bias):
        ...
        phase_h, h = head[:, :, :self.hidden_dim], head[:, :, self.hidden_dim:]
        phase_t, t = tail[:, :, :self.hidden_dim], tail[:, :, self.hidden_dim:]
        phase_r, r = relation[:, :, :self.hidden_dim], relation[:, :, self.hidden_dim:]
        ...
        if args.countries:
            ...
            with torch.no_grad():
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, mode, bias in test_dataset:
                        if args.cuda:
                            positive_sample = positive_sample.cuda()
                            negative_sample = negative_sample.cuda()
                        ...
                        (score, _), _ = model((positive_sample, negative_sample), mode, bias)
