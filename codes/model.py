...
class KGEModel(nn.Module):
    ...
    def forward(self, sample, mode='single'):
        ...
        if mode == 'single':
            ...
            head_community_id = torch.LongTensor([self.communities[int(eid)] for eid in head_part[:, 0]]).cuda()
            tail_community_id = torch.LongTensor([self.communities[int(eid)] for eid in tail_part.view(-1)]).cuda()
            head_community_onehot = torch.index_select(
                self.comMatrix,
                dim=0,
                index=head_community_id,
