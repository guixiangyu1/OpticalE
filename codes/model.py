...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma,
        ...
        self.embedding_range = nn.Parameter(
            torch.Tensor([(9.0 + self.epsilon) / hidden_dim]),
            requires_grad=False
        )
        ...
        self.entity_embedding = nn.Parameter(
            torch.Tensor(np.load('../Optical_margin/OpticalE/models/TestE_YAGO3-10_epoch12lr1.0/entity_embedding.npy')),
            requires_grad=True)

        self.relation_embedding = nn.Parameter(
            torch.Tensor(np.load('../Optical_margin/OpticalE/models/TestE_YAGO3-10_epoch12lr1.0/relation_embedding.npy')),
            requires_grad=False)
    ...
    def TestE_tuning(self, head, relation, tail, mode):
        ...
        score = self.gamma.item() - intensity.sum(dim=2)**0.5
    ...
    def OpticalE_test_ampone(self, head, relation, tail, mode):
        # 震动方向改变，但是强度始终为1
        pi = 3.14159262358979323846
        ...
        intensity = 2 * head_amp.abs() * tail_amp.abs() * a * inference + (head_amp**2 + tail_amp**2)
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        negative_score = negative_score - 15
            ...
            with torch.no_grad():
                for test_dataset in test_dataset_list:
                    for positive_sample, negative_sample, filter_bias, mode in test_dataset:
                        ...
                        if mode == 'head-batch' or mode=='head-batch-test':
