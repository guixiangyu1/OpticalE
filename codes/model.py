...
class KGEModel(nn.Module):
    ...
    def pOpticalE_freelength(self, head, relation, tail, mode):
        ...
        # print(weight.min())
        # print(weight.max())
        weight = torch.cat([torch.ones(weight.shape).cuda()[:,:,:300], weight[:,:,300:]], dim=2)
        print(weight.sum(dim=2)[0 ,0].item())
        bias = 500

        score = self.gamma.item() - (weight * score).sum(dim=2) * 3 / weight.sum(dim=2)
