...
class KGEModel(nn.Module):
    ...
    def OpticalE_dir_ampone(self, head, relation, tail, mode):
        ...
        inference = torch.cos(head_dir - tail_dir)


        intensity = torch.cos((head_phase + rel_phase - tail_phase)) * (inference + 1) + 2
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        if args.negative_adversarial_sampling:
            # In self-adversarial sampling, we do not apply back-propagation on the sampling weight
            # detach() 函数起到了阻断backpropogation的作用
            negative_score = (F.softmax((N_inference - 0.5) * args.adversarial_temperature, dim=1).detach()
