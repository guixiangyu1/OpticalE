...
class KGEModel(nn.Module):
    ...
    def OpticalE_polarization(self, head, relation, tail, mode):
        ...
        a = torch.cos(rel_dir)
        b = torch.sin(rel_dir)
        _R_theta = torch.stack([a,b,-b,a], dim=3).unsqueeze(dim=3).reshape([-1,-1,-1,2,2])
        R_theta = torch.stack([a,-b,b,a], dim=3).unsqueeze(dim=3).reshape([-1,-1,-1,2,2])

        plate_re = torch.cos(delay)
        plate_im = torch.sin(delay)
        zeros = torch.zeros(delay.shape)
        ones = torch.ones(delay.shape)
        plate_re = torch.stack([ones,zeros,zeros,plate_re], dim=3).unsqueeze(dim=3).reshape(-1,-1,-1,2,2)
        plate_im = torch.stack([zeros, zeros, zeros, plate_im], dim=3).unsqueeze(dim=3).reshape(-1, -1, -1, 2, 2)

        a = torch.cos(phase_tail)
        b = torch.sin(phase_tail)
        polarizer = torch.stack([a*a, a*b, a*b, a*a], dim=3).unsqueeze(dim=3).reshape([-1,-1,-1,2,2])
