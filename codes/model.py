...
class KGEModel(nn.Module):
    def __init__(self, model_name, nentity, nrelation, hidden_dim, gamma, 
        ...
        self.relation_dim = hidden_dim*2 if double_relation_embedding else hidden_dim + 1
        ...
        if model_name not in ['TransE', 'DistMult', 'ComplEx', 'RotatE', 'pRotatE', 'OpticalE', 'rOpticalE', 'TransE_periodic',\
                              'TransE_sin', 'TransE_periodic_2D', 'TransE_periodic_amp', 'TransE_periodic_freq','TransE_periodic_dream']:
    ...
    def TransE_periodic_amp(self, head, relation, tail, mode):
        ...
        score = amp_head**2 + amp_tail**2 + 2 * torch.abs(amp_head * amp_tail) * self.triangle_cos(phase)
        # score = torch.abs(score)
        score = self.gamma.item() - score.sum(dim=2)
        return score
