...
class KGEModel(nn.Module):
    ...
    def forward(self, sample, interference=1, mode='single'):
        '''
    ...
    def OpticalE_dir_ampone(self, head, relation, tail, interference, mode):
        ...
        score = self.gamma.item() - intensity * 0.006
    ...
    def train_step(model, optimizer, train_iterator, args):
        ...
        if args.negative_adversarial_sampling:
            # In self-adversarial sampling, we do not apply back-propagation on the sampling weight
            # detach() 函数起到了阻断backpropogation的作用
            neg, _ = torch.min(N_inference, dim=1, keepdim=True)
            neg = N_inference / neg
            negative_score = (F.softmax(neg * args.adversarial_temperature, dim=1).detach()
